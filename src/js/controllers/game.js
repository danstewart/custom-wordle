...
class GameView extends Controller {
	...
	loadSettings() {
		...
		if (queryObject.settings) {
			this.settings = JSON.parse(atob(queryObject.settings));
	...
	render() {
		this.self.replaceChildren(this.grid, this.keyboard);

		if (this.settings.maxTries) {
			let triesLeft = this.settings.maxTries - this.tries;
			this.self.insertAdjacentHTML("afterbegin", html` <p>You have ${triesLeft || "no"} attempts left</p> `);
		}

		if (this.settings.hint) {
			this.self.insertAdjacentHTML(
				"afterbegin",
				html` <p class="hint">Hint: <span class="is-italic">${this.settings.hint}</span></p> `
			);
		}
